...
The scope of the bug bounty program is limited to these [scopes](./#scope). Valid vulnerabilities on any domain not explicitly listed in scope will be accepted, but are ineligible for a reward.&#x20;
...
The [formula](./#miscellaneous) used to calculate the bounty reward can be expressed as follows where `b` is the bounty amount, `C` is the CVSS score, and `N` is a constant that creates a direct correlation between `bmax` and `Cmax:`
